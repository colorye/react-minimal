...
export function render({ routes, originalUrl, path, exact = false, topLevel = true }) {
  if (!routes && global) routes = global.__routes;
  if (!Array.isArray(routes)) return null;
  if (!originalUrl && global) originalUrl = global.__originalUrl || '/';
